# ============================================================
#  CERNIS PRO - Windows ARM64 Build Script
#  Plattform: Windows ARM64 (MSVC)
#  Ergebnis:  CernisPro_1.0.0_arm64-setup.exe (NSIS Installer)
#
#  Aufruf: .\build.ps1
# ============================================================
...
Write-Host "============================================"
Write-Host " CERNIS PRO Windows ARM64 Build"
...
$vsEnv = cmd /c "`"$vcvarsall`" arm64 >nul 2>&1 && set" | Where-Object { $_ -match '=' }
foreach ($line in $vsEnv) {
    $parts = $line -split '=', 2
    [System.Environment]::SetEnvironmentVariable($parts[0], $parts[1], "Process")
}
Write-Host "      VS Build Tools arm64: OK"
...
python -m PyInstaller cernis_windows.spec --noconfirm
...
# Tauri externalBin erwartet: cernis-backend-aarch64-pc-windows-msvc.exe
$TAURI_BIN_NAME = "cernis-backend-aarch64-pc-windows-msvc.exe"
Copy-Item $BACKEND_BIN (Join-Path $TAURI_SRC $TAURI_BIN_NAME) -Force

# Auch in target/aarch64-pc-windows-msvc/release/ ablegen
$TAURI_RELEASE = Join-Path $TAURI_SRC "target\aarch64-pc-windows-msvc\release"
...
$DEST = "C:\Users\karlbach"

$NSIS_DIR = Join-Path $TAURI_SRC "target\aarch64-pc-windows-msvc\release\bundle\nsis"
$INSTALLER = Get-ChildItem -Path $NSIS_DIR -Filter "*.exe" -ErrorAction SilentlyContinue | Select-Object -First 1

if ($INSTALLER) {
    $DEST_FILE = Join-Path $DEST "cernis-pro_${VERSION}_arm64-setup.exe"
