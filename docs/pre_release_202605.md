...
| S7 | Medium | Alerting/SMTP (Korrektheit) | Doppel-Verschlüsselung im `PUT /api/alerts/smtp` Sentinel-Pfad: `main.py:1080–1086` übernimmt beim Sentinel `••••••••` (= „Passwort unverändert") den alten **Cipher** aus `existing.password`, schickt ihn dann aber durch den nachfolgenden `if payload.get("password"): encrypt(...)` — `crypto.encrypt` ist **nicht** idempotent (kein `enc:`-Präfix-Check, `crypto.py:44–48`), also wird der Cipher **doppelt** verschlüsselt. Folge: jeder PW-**erhaltende** Edit (host/port ändern, PW stehen lassen — der Normalfall) korrumpiert das gespeicherte Passwort; `decrypt` liefert beim Versand den inneren Cipher statt des Klartexts → SMTP-Auth scheitert. Aktiver Live-Bug (anders als das tote `fire_alert`). Im Rewrite (A.4b) gefunden + empirisch reproduziert. **In v2 geheilt** (A.4b `SettingsSmtpConfigAdapter.save`: Sentinel → alten Cipher 1:1 übernehmen OHNE re-encrypt, nur echtes neues Klartext-PW genau 1× `encrypt`); im sterbenden `main.py` bewusst **nicht** gepatcht (stirbt mit Einstiegspunkt-Wechsel P2.3) |

> **Hinweis:** S1–S5 stammen aus dem externen Security-Review (Teil 1); S6 wurde im Rewrite (P2.1c) gefunden, S7 im Rewrite (A.4b). Weitere Findings (Teile 2+) werden im Audit der Phase 4 systematisch erhoben.
