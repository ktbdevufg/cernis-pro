...
def generate_report(
    ...
    # Styles for sections outside dark tables (white page background)
    C_BLACK = colors.HexColor("#111111")
    sBodyBlk = S("BB", fontName="Helvetica", fontSize=9, textColor=C_BLACK, spaceAfter=4)
    sMonoBlk = S("MB", fontName="Courier", fontSize=8.5, textColor=C_BLACK)
    ...
    if risky or unknown_devs:
        ...
        if risky:
            story.append(Paragraph(
                f"<b>{len(risky)} host(s)</b> with potentially risky open ports:", sBodyBlk))
            for h in risky[:10]:
    ...
    # ── Footer ────────────────────────────────────────────────
    story.append(Spacer(1, 8*mm))
    story.append(HRFlowable(width=W, thickness=0.5, color=colors.HexColor("#888888")))
    story.append(Paragraph(
        f"Generated by CERNIS PRO v1.0.0  ·  {now}  ·  Scan ID: {scan.get('id', '—')}",
        S("F", fontName="Helvetica", fontSize=8, textColor=C_BLACK,
